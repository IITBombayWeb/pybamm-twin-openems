...
# Function to generate random variations for cell parameters
def generate_variations():
    capacity_variation = random.uniform(0.9, 1.1)  # ±10% variation
    contact_resistance_variation = random.uniform(0.95, 1.05)  # ±5% variation
    return capacity_variation, contact_resistance_variation

# Define Pybamm setup with variations
def mySPMEsim(parameter_values=None, capacity_variation=1.0, contact_resistance_variation=1.0):
    ...
    # Set up parameter values with variations
    if parameter_values is None:
        param = pybamm.ParameterValues("Chen2020")
    else:
        param = parameter_values.copy()
    
    param["Nominal cell capacity [A.h]"] *= capacity_variation
    param["Contact resistance [Ohm]"] *= contact_resistance_variation
...
def update_registers(context):
    ...
    while True:
        # Generate variations for each cell
        capacity_variation, contact_resistance_variation = generate_variations()

        # Run Pybamm simulation with variations
        sim_output = lp.solve(
            netlist=netlist,
            parameter_values=parameter_values,
            experiment=experiment,
            sim_func=lambda pv: mySPMEsim(pv, capacity_variation, contact_resistance_variation),
        ...
        for i in range(len(sim_time)):
            ...
            if i % 10 == 0:  # Print every 1000th value
            ...
            time.sleep(0.1)  # Sleep for 100ms before updating again

...
def update_registers(context):
    ...
    while True:
        ...
        for i in range(len(sim_time)):
            ...
            # Calculate battery power as voltage * current
            battery_power = voltage_last * current_last
            
            # Simulate production and consumption data
            production = random.uniform(0.8, 1.2) * battery_power  # Example production value
            consumption = random.uniform(0.5, 1.0) * battery_power  # Example consumption value

            # Convert data to Modbus register format
            values = [
                int(production * 1000),  # Example production conversion
                int(consumption * 1000), # Example consumption conversion
                int(battery_power * 100), # Example battery power conversion
                int(soc_last * 100),      # Example SoC conversion
            ]
            
            if i % 10 == 0:  
                # Update Modbus registers
                context[0].setValues(3, 1000, [values[0]])
                print(f"Set production value at address 1000: {values[0]}")
                context[0].setValues(3, 1001, [values[1]])
                print(f"Set consumption value at address 1001: {values[1]}")
                context[0].setValues(3, 1002, [values[2]])
                print(f"Set battery power value at address 1002: {values[2]}")
                context[0].setValues(3, 1003, [values[3]])
                print(f"Set SoC value at address 1003: {values[3]}")
